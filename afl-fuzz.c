...
#include "llvm_mode/state-tracer.h"

static s32 shm_state_id;
volatile struct state_shared * state_shared_ptr;
...
// AFLNet state sequences are saved for debugging/data-analysis purposes
unsigned int aflnet_state_count = 0;
unsigned int* aflnet_state_sequence = NULL;

#endif

#define LOG_STATE_TRACER_FILE "./state-tracer-afl.log"
//#define LOG_STATE_TRACER

#ifdef LOG_STATE_TRACER
FILE * log_state_tracer;
#endif
...
#ifdef GRAPHVIZ_AFLNET_STATES
  agattr(ipsm, AGNODE, "xlabel", "");
  agattr(ipsm, AGEDGE, "label", "");
#endif
  ...
  if ((!dry_run && is_repeatable) || (dry_run && is_interesting)) {
    ...
    save_kl_messages_to_file(kl_messages, fname, 1, messages_sent);
...
#ifdef GRAPHVIZ_EDGE_LABELS

		char ** buf_new = NULL;
        char ** buf_prev = NULL;

        buf_new = (char **)ck_alloc(sizeof(char*)*state_count);


		FILE * fp_new = fopen(fname, "rb");

		unsigned int msg_i = 0;
		while(!feof(fp_new)) {

			if(msg_i >= state_count) break;

			unsigned int size;
			if (fread(&size, sizeof(unsigned int), 1, fp_new) > 0) {

				buf_new[msg_i] = (char *)ck_alloc(size);
				fread(buf_new[msg_i], size, 1, fp_new);

				buf_new[msg_i][size-1] = '\0';
        ...
        if(!dry_run) {
          ...
          buf_prev = (char **)ck_alloc(sizeof(char*)*state_count);

          FILE * fp_prev = fopen(q->fname, "rb");

          msg_i = 0;
          while(!feof(fp_prev)) {

              if(msg_i >= state_count || msg_i >= q->region_count) break;

              unsigned int region_size = q->regions[msg_i].end_byte - q->regions[msg_i].start_byte + 1;

              buf_prev[msg_i] = (char *)ck_alloc(region_size);
              fread(buf_prev[msg_i], region_size, 1, fp_prev);

              buf_prev[msg_i][region_size-1] = '\0';
    ...
    if (state_count > 1) {
...
#ifdef GRAPHVIZ_AFLNET_STATES
      unsigned int prevAFLNetStateID = aflnet_state_sequence[0];
#endif
      ...
      for(i=1; i < state_count; i++) {
...
#ifdef GRAPHVIZ_AFLNET_STATES
        unsigned int curAFLNetStateID = aflnet_state_sequence[i];
#endif

        char fromState[STATE_STR_LEN], toState[STATE_STR_LEN];

#if !defined(GRAPHVIZ_AFLNET_STATES) || !defined(GRAPHVIZ_AFLNET_INSIDE_NODE)
        snprintf(fromState, STATE_STR_LEN, "%d", prevStateID);
        snprintf(toState, STATE_STR_LEN, "%d", curStateID);
#else
        snprintf(fromState, STATE_STR_LEN, "%d", prevAFLNetStateID);
        snprintf(toState, STATE_STR_LEN, "%d", curAFLNetStateID);
#endif
...
#if defined(GRAPHVIZ_AFLNET_STATES)
#if defined(GRAPHVIZ_AFLNET_INSIDE_NODE)

        char from_label_aflnet[10*STATE_STR_LEN];
        snprintf(from_label_aflnet, STATE_STR_LEN, "%d", prevStateID);
        agset(from,"xlabel", from_label_aflnet);

#else

        char from_label_aflnet[10*STATE_STR_LEN];
        snprintf(from_label_aflnet, STATE_STR_LEN, "%d", prevAFLNetStateID);
        agset(from,"xlabel", from_label_aflnet);
          ...
          char * prev_from_label = agget(from,"xlabel");

          if(prev_from_label) {
            if(strstr(prev_from_label, from_label_aflnet)==NULL) {
              snprintf(from_label_aflnet, 10*STATE_STR_LEN, "%s,%d", prev_from_label, prevStateID);
              agset(from,"xlabel", from_label_aflnet);
            }
          }
          ...
          if(prev_from_label) {
            if(strstr(prev_from_label, from_label_aflnet)==NULL) {
              snprintf(from_label_aflnet, 10*STATE_STR_LEN, "%s,%d", prev_from_label, prevAFLNetStateID);
              agset(from,"xlabel", from_label_aflnet);
            }
          }
        ...
        char to_label_aflnet[10*STATE_STR_LEN];
        snprintf(to_label_aflnet, STATE_STR_LEN, "%d", curStateID);
        agset(to,"xlabel", to_label_aflnet);

#else

        char to_label_aflnet[10*STATE_STR_LEN];
        snprintf(to_label_aflnet, STATE_STR_LEN, "%d", curAFLNetStateID);
        agset(to,"xlabel", to_label_aflnet);
          ...
          char * prev_to_label = agget(to,"xlabel");

          if(prev_to_label) {
            if(strstr(prev_to_label, to_label_aflnet)==NULL) {
              snprintf(to_label_aflnet, 10*STATE_STR_LEN, "%s,%d", prev_to_label, curStateID);
              agset(to,"xlabel", to_label_aflnet);
            }
          }
          ...
          if(prev_to_label) {
            if(strstr(prev_to_label, to_label_aflnet)==NULL) {
              snprintf(to_label_aflnet, 10*STATE_STR_LEN, "%s,%d", prev_to_label, curAFLNetStateID);
              agset(to,"xlabel", to_label_aflnet);
            }
          }
...
// Always print the first chars
#define PREFIX_LEN 5

// Print a fixed amount of chars after the first differing ones
#define DIFF_LEN 10

          if(i>1) {

            unsigned int msg_i = i-2;

            unsigned int msg_len = strlen(buf_new[msg_i]);
            unsigned int label_offset = 0;

            unsigned int buf_len = PREFIX_LEN + 4 + DIFF_LEN + 4;
            char * label = ck_alloc(buf_len);


            if(msg_len < buf_len) {

              memcpy(label, buf_new[msg_i], msg_len);

            }
            else {

              memcpy(label, buf_new[msg_i], PREFIX_LEN);
              label_offset += PREFIX_LEN;

              if(!dry_run) {

                unsigned int msg_len_prev = strlen(buf_prev[msg_i]);

                unsigned int shortest_len = (msg_len < msg_len_prev) ? msg_len : msg_len_prev;
                int found = -1;
                for(int j=0; j<shortest_len; j++) {

                  if(buf_new[msg_i][j] != buf_prev[msg_i][j]) {
                    found = j;
                    break;
                  }
                }
                ...
                if(found != -1) {
                  ...
                  if(found > PREFIX_LEN+3) {
                    memcpy(label + label_offset, "...", 3);
                    label_offset += 3;
                  }

                  memcpy(label + label_offset, buf_new[msg_i] + found, DIFF_LEN);
                  label_offset += DIFF_LEN;

                }

              }

              memcpy(label + label_offset, "...", 4);
            ...
            int j=0;
            while(j<buf_len && label[j] != '\0') {

              if(label[j] == '\n' || label[j] == '\r') {
                label[j] = ' ';
              }
              else if(label[j] < 32 || label[j] >= 127) {
                label[j] = '*';
              }
            ...
            char * prev_edge_label = agget(edge,"label");

            if(prev_edge_label && *prev_edge_label != '\0') {

#define APPENDED_LEN 100
              char appended_label[APPENDED_LEN];

              if(strstr(prev_edge_label, label)==NULL && strlen(prev_edge_label) + strlen(label) + 2 < APPENDED_LEN) {

                snprintf(appended_label, APPENDED_LEN, "%s, %s", prev_edge_label, label);
                agset(edge,"label", appended_label);
              }
...
#ifdef GRAPHVIZ_AFLNET_STATES
        prevAFLNetStateID = curAFLNetStateID;
#endif
      }
    }
...
static void remove_shm(void) {
  ...
  shmctl(mvp_calibration_shm_id, IPC_RMID, NULL);
#endif

#ifdef LOG_STATE_TRACER
  fclose(log_state_tracer);
#endif
}
...
EXP_ST void setup_shm(void) {
  ...
  shm_state_id = shmget(IPC_PRIVATE, sizeof(struct state_shared), IPC_CREAT | IPC_EXCL | 0600);

  if (shm_state_id < 0) PFATAL("shmget() failed");

  u8 * shm_state_str = alloc_printf("%d", shm_state_id);

  setenv(SHM_STATE_ENV_VAR, shm_state_str, 1);

  ck_free(shm_state_str);

  state_shared_ptr = shmat(shm_state_id, NULL, 0);

  if (!state_shared_ptr) PFATAL("shmat() failed");


#ifndef DISABLE_TLSH_CALIBRATION

  mvp_calibration_shm_id = shmget(IPC_PRIVATE, sizeof(struct calibration), IPC_CREAT | IPC_EXCL | 0600);

  if (mvp_calibration_shm_id < 0) PFATAL("shmget() failed");

  calib_shm = shmat(mvp_calibration_shm_id, NULL, 0);

  if (!calib_shm) PFATAL("shmat() failed");

  calib_shm->enabled = 1;
  calib_shm->initialized = 0;
  calib_shm->ref_len = 0;
  calib_shm->dist_len = 0;
  calib_shm->mvp_radius = 1;
  calib_shm->rate_limit = 0;

  char mvp_calibration_shm_str[10];
  sprintf(mvp_calibration_shm_str, "%d", mvp_calibration_shm_id);
  setenv(MVP_CALIBRATION_ENV_VAR, mvp_calibration_shm_str, 1 /*overwrite*/);

#endif


#ifdef AFL_OUTDIR
  setenv(AFL_OUTDIR_ENV_VAR, out_dir, 1);
#endif


#ifdef LOG_STATE_TRACER
  char log_state_fname[PATH_MAX];
  snprintf(log_state_fname, PATH_MAX, "%s/%s", out_dir, LOG_STATE_TRACER_FILE);

  log_state_tracer = fopen(log_state_fname, "w+");
#endif
}
...
void init_ipsm(char** argv) {

  OKF("Initializing IPSM by running the seed inputs");
  ...
  while (q) {
    ...
    u8* use_mem = ck_alloc_nozero(q->len);
...
static int compare_tlsh_diff(const void* p1, const void* p2) {
...
void tlsh_set_distance() {

  if(calib_shm->enabled == 0)
    return;

  calib_shm->enabled = 0;

  if(calib_shm->dist_len == 0) {

    char * mvp_radius_str = getenv(MVP_RADIUS_ENV_VAR);

    if(mvp_radius_str) {

      errno = 0;
      int radius = strtol(mvp_radius_str, NULL, 10);

      if(errno == 0) {

        calib_shm->mvp_radius = radius;

        WARNF("No data from TLSH calibration runs, setting distance threshold from " MVP_RADIUS_ENV_VAR " (%d)", calib_shm->mvp_radius);

      } else {

        calib_shm->mvp_radius = 100;

        WARNF("Invalid " MVP_RADIUS_ENV_VAR " (%s), setting distance threshold to default (%d)", mvp_radius_str, calib_shm->mvp_radius);
      ...
      WARNF("No data from TLSH calibration runs, setting distance threshold to default (%d)", calib_shm->mvp_radius);
    }
  ...
  for(int i=0; i<calib_shm->dist_len; i++) {
    if(calib_shm->dist[i] > max_dist)
      max_dist = calib_shm->dist[i];
  }

  tlsh_diff = max_dist;
  */

  if(tlsh_diff == 0 || tlsh_diff % 5 != 0) {

    tlsh_diff = (5 - tlsh_diff % 5) + tlsh_diff;
  }

  OKF("Setting TLSH distance threshold to %d", tlsh_diff);


  // Note: we use an SHM instead of an environment var,
  // since the child forkserver process has already been
  // started at this stage

  calib_shm->mvp_radius = tlsh_diff;

  /*
  char mvp_radius_str[10];
  sprintf(mvp_radius_str, "%d", tlsh_diff);
  setenv(MVP_RADIUS_ENV_VAR, mvp_radius_str, 1);
  */
...
#endif // DISABLE_TLSH_CALIBRATION
...
unsigned int* extract_response_state_tracer(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref) {
  ...
  *state_count_ref = state_tracer_count_all;
  ...
  fprintf(log_state_tracer, "state_sequence: ");

  for(int i=0; i<*state_count_ref; i++)
  ...
  for(int i=0; i<orig_state_count; i++)
...
#ifndef DISABLE_TLSH_CALIBRATION
    // re-initialize calibration for the next input
    calib_shm->initialized = 0;
    calib_shm->ref_len = 0;
#endif
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:e:P:KEq:s:RFc:l:u:")) > 0)
    ...
    switch (opt) {
      ...
      case 'e': /* receive/send socket timeout determining time waited for each response */
        if (send_delay) FATAL("Multiple -e options not supported");

        if (sscanf(optarg, "%u", &send_delay_us) < 1 || optarg[0] == '-') FATAL("Bad syntax used for -e");
        send_delay = 1;
        break;
  ...
  if (getenv("AFL_FAST_CAL"))      fast_cal         = atoi(getenv("AFL_FAST_CAL"));
